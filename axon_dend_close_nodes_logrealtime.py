...
Edit on Mar 25:
    ...
    3. Redirect output messages to a log file for real-time checking.
"""
import os
...
def find_nodes_closeby(axon_skel_set, dend_skel_set, max_distance, log_file,save_dir):
            ...
            for dend_skel in dend_skel_set.flattened_trees():
                try:
                    if axon_nuc_id not in dend_skel.name:
                        ...
                        axon_skel_nodes = axon_skel.get_node_positions() * np.array([3.75, 3.75, 50]) / 1000  # unit in um
                        ...
                        if dend_index_unique.any():
                            ...
                            log_file.write(f"Finished axon {axon_nuc_id} dendrite {dend_nuc_id}\n")
                            log_file.flush()  # ensure the message is written immediately
                except Exception as e:
                    log_file.write(f"Error processing dendrite file {dend_skel.name}: {e}\n")
                    log_file.flush()
            axon_skel_set.save(save_dir)
        except Exception as e:
            log_file.write(f"Error processing axon file {axon_skel.name}: {e}\n")
            log_file.flush()

# Define file names (update these with the actual filenames)
file_name_axon = "one_large_axon.nml"  # replace with your axon file name
file_name_dend = "Consensus_Dend_032425.nml"    # replace with your dendrite file name
save_name = "Overlap_Axon_032525_2um_single_long_axon.nml"
...
save_path = Path(os.path.join(main_path, save_name))
# Open a log file to capture progress messages in real time
log_file_path = Path(os.path.join(main_path, "process_log.txt"))
with open(log_file_path, "w") as log_file:
    find_nodes_closeby(axon_skel_set, dend_skel_set, max_distance, log_file,save_path)

axon_skel_set.save(save_path)

# Save flattened groups information to a text file
